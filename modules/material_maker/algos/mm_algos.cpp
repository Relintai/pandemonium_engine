...
#include "mm_algos.h"
 ...
 Vector2 MMAlgos::transform(const Vector2 &uv, const Vector2 &translate, const float rotate, const Vector2 &scale, const bool repeat) {
   Vector2 rv = Vector2();
  uv -= translate;
  uv -= Vector2(0.5, 0.5);
  ...
  for (int i = 0; i <  s; ++i) { //i in range(0, s)
   ...
   if (x < get_data_pos(i, data)) {
    ...
    //			var b : String = "mix(%s, %s, %s)" % [ pc(name, i), pc(name, i+1), dx ];
     Color b = lerp(get_data_color(i - 1, data), get_data_color(i - 1, data), dx);

    if (i == 1) {
     //				var c : String = "mix(%s, %s, (x-%s)/(%s-%s))" % [ pc(name, i+1), pc(name, i+2), pv(name, i+1), pv(name, i+2), pv(name, i+1) ];
      Color c = lerp(get_data_color(i + 1, data), get_data_color(i + 2, data), (x - get_data_pos(i + 1, data))/(get_data_pos(i + 2, data) - get_data_pos(i + 1, data)));
    ...
    //			var a : String = "mix(%s, %s, (x-%s)/(%s-%s))" % [ pc(name, i-1), pc(name, i),  pv(name, i-1), pv(name, i), pv(name, i-1) ];
     Color a = lerp(get_data_color(i - 1, data), get_data_color(i, data), (x - get_data_pos(i - 1, data)) / (get_data_pos(i, data) - get_data_pos(i - 1, data)));
    //			if i < s-1:;
    ...
    if (i < s - 1) {
     ...
     //				var ac : String = "mix("+a+", "+c+", 0.5-0.5*cos(3.14159265359*"+dx+"))";
      Color ac = lerp(a, c, 0.5-0.5*cos(3.14159265359 * dx));
 ...
 float MMAlgos::anisotropic(const Vector2 &uv, const Vector2 &size, const float pseed, const float smoothness, const float interpolation) {
  ...
  s2xy.y += xy.y;
   ...
   float smooth_mix = smoothstep(0.0, 1.0, mixer);
   float linear = lerp(f0, f1, mixer);
   float smoothed = lerp(f0, f1, smooth_mix);
 ...
 Color MMAlgos::brick(const Vector2 &uv, const Vector2 &bmin, const Vector2 &bmax, const float mortar, const float pround, const float bevel) {
  ...
  //	var tiled_brick_pos : Vector2 = Vector2(bmin.x - 1.0 * floor(bmin.x / 1.0), bmin.y - 1.0 * floor(bmin.y / 1.0));
   float tiled_brick_pos_x = bmin.x - 1.0 * floor(bmin.x / 1.0);
   float tiled_brick_pos_y = bmin.y - 1.0 * floor(bmin.y / 1.0);
  //vec2 tiled_brick_pos = mod(bmin, vec2(1.0, 1.0));
 ...
 Color MMAlgos::bricks_hb(const Vector2 &uv, const Vector2 &count, const float repeat, const float offset) {
   float pc = count.x + count.y;
   float c = pc * repeat;
   Vector2 corner = Vector2(floor(uv.x * c), floor(uv.y * c));
 ...
 Color MMAlgos::bricks_bw(const Vector2 &uv, const Vector2 &count, const float repeat, const float offset) {
   ...
   float mc = max(c.x, c.y);
   Vector2 corner1 = Vector2(floor(uv.x * c.x), floor(uv.y * c.y));
   Vector2 corner2 = Vector2(count.x * floor(repeat* 2.0 * uv.x), count.y * floor(repeat * 2.0 * uv.y));
   Vector2 tmp = Vector2(floor(repeat * 2.0 * uv.x), floor(repeat * 2.0 * uv.y));
 ...
 Color MMAlgos::bricks_sb(const Vector2 &uv, const Vector2 &count, const float repeat, const float offset) {
   ...
   float mc = max(c.x, c.y);
   Vector2 corner1 = Vector2(floor(uv.x * c.x), floor(uv.y * c.y));
   Vector2 corner2 = (count + Vector2(1, 1)) * Vector2(floor(repeat * uv.x), floor(repeat * uv.y));
