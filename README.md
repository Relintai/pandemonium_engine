...
On top of this some github actions now takes over 40 minutes to run.

I'll just assume it's due to the vibe coding (both by lots of people hammering infra and internally), so in order to make sure
...
Even though this engine is practically unknown, and it's not really getting patch
submissions as of now (realistically it won't get submissions probably ever),
to be on the safe side I'm adopting the following policy (very similar to zig's):
